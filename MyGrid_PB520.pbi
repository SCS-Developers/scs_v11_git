...
Enumeration                             ; Row/Col attributes
  ...
  #MyGrid_Att_LostFocusRow
  #MyGrid_Att_LostFocusCol
  
  #MyGrid_Att_GadgetRowScroll         ; Gadget nbr of Row-Scroll - in case
  #MyGrid_Att_GadgetColScroll         ; Gadget nbr of Col-Scroll
  #MyGrid_Att_RowScrollPageSize
  #MyGrid_Att_ColScrollPageSize
EndEnumeration

; the following moved to Constants.pbi so the event numbers do not clash with SCS event numbers
; Enumeration #PB_Event_FirstCustomValue
...
Procedure.i MyDrawText(txt.s,x,y,width,height,algn=0,wrap=0)
  ...
  If w <= aw 
    ...
    y1 = y + my + ((ah - h)/2)
    DrawText(x1,y1,txt)
    ; debugMsg(sProcName, "MyGrid\DrawText(" + x1 + ", " + y1 + ", " + #DQUOTE$ + txt + #DQUOTE$ + ")")
    ProcedureReturn
  Else
    x1 = x + mx : x2 = x1 + aw
    If wrap = #False
      DrawText(x1,y1, Mid(txt, 1, PosTextToWidth(txt, aw)))
      ; debugMsg(sProcName, "MyGrid\DrawText(" + x1 + ", " + y1 + ", " + #DQUOTE$ + Mid(txt, 1, PosTextToWidth(txt, aw)) + #DQUOTE$ + ")")
    Else
...
Procedure.i MyDrawButton(x,y,w,h,bClr)
...
Procedure.i _MyGrid_AddAreaRow(*mg._MyGrid_Type, Row, Y, H)
  ; Debug "_MyGrid_AddAreaRow Row=" + Row
...
Procedure.i _MyGrid_ChangeMouse(*mg._MyGrid_Type, x, y)
  ...
  *mg\DeltaY = WindowMouseY(*mg\Window) - y
  ...
  If _MyGrid_OverCellArea(*mg, x, y)
    ...
    If (*mg\LstStyle()\CellType <> #MyGrid_CellType_Normal) Or (*mg\LstStyle()\Editable = #False)
    ...
    Else
      ...
      SetGadgetAttribute(*mg\Gadget, #PB_Canvas_Cursor,#PB_Cursor_IBeam)
    EndIf
    ProcedureReturn
  EndIf
  SetGadgetAttribute(*mg\Gadget, #PB_Canvas_Cursor,#PB_Cursor_Default)
...
; Debug "(S) Row=" + Row + ", wrd=" + wrd
    ...
    Case #MyGrid_CellType_Button
      ;MyDrawButton(X+2,Y+2,W-4,H-4,SBColor)
      MyDrawButton(X+1,Y+1,W-2,H-2,SBColor)
      DrawingMode(#PB_2DDrawing_Transparent)
      If IsFont(SFont) : DrawingFont(FontID(SFont)) : EndIf
      FrontColor(SFColor)
      MyDrawText(wrd, X,Y,W,H, SAlign, *mg\WrapText)
  ...
  Else
    ...
    If mlt < 0
      ...
      If ar >= 0 And ac >= 0
        ...
        h = *mg\LstAreaRow()\Height
        c = *mg\Color_FocusBorder
        ; Debug "(b) _MyGrid_DrawFocus x=" + x + ", ar=" + ar + ", ac=" + ac + ", *mg\Row=" + *mg\Row + ", *mg\Col=" + *mg\Col
        Box(x, y, w, h, c)
        Box(x+1, y+1, w-2, h-2, c)
      EndIf
...
Procedure.i _MyGrid_MoveFocus(*mg._MyGrid_Type, Row, Col)
  ; PROCNAME("_MyGrid_MoveFocus")
  If StartDrawing(CanvasOutput(*mg\Gadget))
    ; Debug "_MyGrid_MoveFocus calling _MyGrid_DrawCell(*mg, " + *mg\Row + ", " + *mg\Col + ")"
    _MyGrid_DrawCell(*mg, *mg\Row, *mg\Col)
    *mg\Row = Row
    *mg\Col = Col
    ; Debug sProcName + " calling _MyGrid_DrawFocus(*mg)"
...
Procedure.i _MyGrid_DrawCurrentCell(*mg._MyGrid_Type)
  ; PROCNAME("_MyGrid_DrawCurrentCell")
  If StartDrawing(CanvasOutput(*mg\Gadget))
    ; Debug "_MyGrid_DrawCurrentCell calling _MyGrid_DrawCell(*mg, " + *mg\Row + ", " + *mg\Col + ")"
    _MyGrid_DrawCell(*mg, *mg\Row, *mg\Col)
    ; Debug sProcName + " calling _MyGrid_DrawFocus(*mg)"
    _MyGrid_DrawFocus(*mg)
    StopDrawing()
...
Procedure   _MyGrid_Draw(*mg._MyGrid_Type)
  ; PROCNAME("_MyGrid_Draw")
  Protected WW,HH,x,y,w,h,Row,Col,clr,area, ar, ac, t.q
...
;   debugMsg(sProcName, "(canvas) GadgetX(" + *mg\Gadget + ")=" + GadgetX(*mg\Gadget) + ", GadgetY(" + *mg\Gadget + ")=" + GadgetY(*mg\Gadget) +
;                       ", GadgetWidth(" + *mg\Gadget + ")=" + GadgetWidth(*mg\Gadget) + ", GadgetHeight(" + *mg\Gadget + ")=" + GadgetHeight(*mg\Gadget))
    ...
    Case #MyGrid_Move_Focus
      ...
      If *mg\TopRow <> i
        ...
        *mg\Row = Row
        ProcedureReturn #True
      Else
        ; Debug sProcName + " calling _MyGrid_MoveFocus()"
...
Procedure.i _MyGrid_ExtendBlock_XY(*mg._MyGrid_Type, X,Y)
  ; PROCNAME("_MyGrid_ExtendBlock_XY")
  ...
  If X > *mg\GadgetW
    ...
    ; Debug sProcName + " calling _MyGrid_MoveRight() (a)"
  ...
  If (Col > 0) And (*mg\Col2 <> Col)
    ...
    If Col > *mg\Col2
      ; Debug sProcName + " calling _MyGrid_MoveRight() (b)"
      _MyGrid_MoveRight(*mg, xStep, #MyGrid_Move_Block)
    EndIf
    If Col < *mg\Col2
      _MyGrid_MoveLeft(*mg, xStep, #MyGrid_Move_Block)
    EndIf
    *mg\Col2 = Col
  EndIf
...
Procedure.i _MyGrid_AdjustScrolls(*mg._MyGrid_Type)
  ...
  If IsGadget(*mg\ColScroll)
    If *mg\LastTopCol <= #MyGrid_Scroll_Max
      ...
      scrPage = *mg\ColScrollPageSize ; #MyGrid_Scroll_PageSize
  ...
  If IsGadget(*mg\RowScroll)
    If *mg\LastTopRow <= #MyGrid_Scroll_Max
      ; LastTop = scrMax - scrPage + 1  ==>  scrMax = LastTop + scrPage - 1
      ; we have full match:  CurTop = 1 * CurState
      scrPage = *mg\RowScrollPageSize ; #MyGrid_Scroll_PageSize
...
Procedure.i _MyGrid_ManageEdit(*mg._MyGrid_Type, ky.s, EnterPressed, SimpleClick)
  ...
  If *mg\LstStyle()\Editable = #False
    ; debugMsg(sProcName, "ProcedureReturn because *mg\LstStyle()\Editable=#False")
    ProcedureReturn #False
  EndIf
  ...
  If ar < 0 Or ac < 0
    ; debugMsg(sProcName, "ProcedureReturn because ar=" + ar + ", ac=" + ac)
    ProcedureReturn #False
  EndIf
  ...
  Select *mg\LstStyle()\CellType
    ...
    Case #MyGrid_CellType_Checkbox
      ...
      If ky = " "  Or EnterPressed Or SimpleClick 
        ...
        EndIf
        ; Debug "(a) calling _MyGrid_DrawCurrentCell(*mg)"
        _MyGrid_DrawCurrentCell(*mg)
        ; added MD 13Jul2018 11.7.2aa because we need to be able to take some action when a checkbox is clicked in fmOptions video driver settings
        *mg\ClickedRow = r : *mg\ClickedCol = c
        PostEvent(#MyGrid_Event_Click, *mg\Window, *mg\Gadget) ; throw an event in the loop
        ; end added MD 13Jul2018 11.7.2aa
      EndIf
      ProcedureReturn #False
    ...
    Case #MyGrid_CellType_Normal
      If SimpleClick
        ; debugMsg(sProcName, "#MyGrid_CellType_Normal: ProcedureReturn because SimpleClick=#True")
        ProcedureReturn #False ; getting focus is not entring edit mode!
      EndIf
      Select editMode
  ...
  If gEvt > #_MyGrid_ExitEdit
    ...
    If nTxt <> oTxt
      ...
      ; Debug "(b) calling _MyGrid_DrawCurrentCell(*mg)"
      _MyGrid_DrawCurrentCell(*mg)
    EndIf
  EndIf
  If _MyGrid_IsValidCell(*mg, nr, nc)
    ; Debug sProcName + " calling _MyGrid_MoveFocus()"
    _MyGrid_MoveFocus(*mg, nr, nc)
  EndIf
  ...
  Select evMn
    ...
    Case #_MyGrid_ExitEdit_Vld_PUp: ProcedureReturn _MyGrid_MoveUp(*mg, *mg\RowScrollPageSize) ; #MyGrid_Scroll_PageSize)
    Case #_MyGrid_ExitEdit_Vld_PDn: ProcedureReturn _MyGrid_MoveDown(*mg, *mg\RowScrollPageSize) ; #MyGrid_Scroll_PageSize)
  EndSelect
...
Procedure.i _MyGrid_Reset(*mg._MyGrid_Type, Rows, Cols)
  ...
  *mg\Col2                = 0
  ...
  For i=1 To cols
    _MyGrid_SetCellText(*mg, 0, i, "Col " + i)
  Next
...
Procedure.i _MyGrid_ResetRows(*mg._MyGrid_Type, Rows)
  ...
  If Rows < 0 : Rows = 0  : EndIf
  ; Debug "_MyGrid_ResetRows Rows=" + Rows
...
Procedure.i MyGrid_MoveRight(Gdt, xStep, moveWhat)
  ; PROCNAME("MyGrid_MoveRight")
  ; Debug sProcName + " calling _MyGrid_MoveRight()"
  If _MyGrid_MoveRight(GetGadgetData(Gdt), xStep, moveWhat)
    _MyGrid_Draw(GetGadgetData(Gdt))
  EndIf
EndProcedure

Procedure.i MyGrid_ShowCell(Gdt.i, Row, Col, SetCellFocus = #False)
  ; PROCNAME("MyGrid_ShowCell")
...
Macro MyGrid_FocusCell(Gdt, Row, Col)
  ...
  ; Debug sProcName + " [MyGrid_FocusCell] (a): Row=" + Row + ", Col=" + Col + ", *mg\FocusRow=" + *mg\FocusRow + ", *mg\FocusCol=" + *mg\FocusCol + ", *mg\LostFocusRow=" + *mg\LostFocusRow + ", *mg\LostFocusCol=" + *mg\LostFocusCol
  If (Row <> *mg\FocusRow) Or (Col <> *mg\FocusCol)
    *mg\LostFocusRow = *mg\FocusRow
    *mg\LostFocusCol = *mg\FocusCol
    *mg\FocusRow = Row
    *mg\FocusCol = Col
    ; Debug sProcName + " [MyGrid_FocusCell] (b): Row=" + Row + ", Col=" + Col + ", *mg\FocusRow=" + *mg\FocusRow + ", *mg\FocusCol=" + *mg\FocusCol + ", *mg\LostFocusRow=" + *mg\LostFocusRow + ", *mg\LostFocusCol=" + *mg\LostFocusCol
    If (*mg\LostFocusRow > 0) And (*mg\LostFocusCol > 0)
      ; Debug ">>>>>>>>>>>>>>>>> lost focus"
      PostEvent(#MyGrid_Event_LostFocus, *mg\Window, *mg\Gadget) ; throw a 'lost focus' event in the loop for the previously focussed cell
    EndIf
    ; Debug sProcName + " [MyGrid_FocusCell]: calling MyGrid_ShowCell(gdt, " + Row + ", " + Col + ")"
    MyGrid_ShowCell(Gdt, Row, Col, #True)
    PostEvent(#MyGrid_Event_Focus, *mg\Window, *mg\Gadget) ; throw a 'focus' event in the loop for the now selected cell
  EndIf
EndMacro
...
Procedure.i MyGrid_GetAttribute(Gdt.i, Attribute = #MyGrid_Att_Row, RowOrCol = 0)
  ...
  Select Attribute
    ...
    Case #MyGrid_Att_LostFocusRow : ProcedureReturn *mg\LostFocusRow
    Case #MyGrid_Att_LostFocusCol : ProcedureReturn *mg\LostFocusCol
      
    Case #MyGrid_Att_GadgetRowScroll    : ProcedureReturn *mg\RowScroll
    Case #MyGrid_Att_GadgetColScroll    : ProcedureReturn *mg\ColScroll
    Case #MyGrid_Att_RowScrollPageSize    : ProcedureReturn *mg\RowScrollPageSize
    Case #MyGrid_Att_ColScrollPageSize    : ProcedureReturn *mg\ColScrollPageSize
...
Procedure MyGrid_SetAttribute(Gdt.i, Attribute, NewValue)
  Protected *mg._MyGrid_Type = GetGadgetData(Gdt)
  
  Select Attribute
    Case #MyGrid_Att_RowScrollPageSize
      *mg\RowScrollPageSize = NewValue
    Case #MyGrid_Att_ColScrollPageSize
      *mg\ColScrollPageSize = NewValue
  EndSelect
...
Procedure   MyGrid_Resize(Gdt.i, X,Y,W,H)
  ...
  ; debugMsg(sProcName, "X=" + X + ", Y=" + Y + ", W=" + W + ", H=" + H)
  ...
  If IsGadget(*mg\ColScroll)
    ResizeGadget( *mg\ColScroll, X, Y + H, W, #MyGrid_Scroll_Width)
    ; debugMsg(sProcName, "(ColScroll) ResizeGadget(" + *mg\ColScroll + ", " + X + ", " + Str(Y + H) + ", " + W + ", " + #MyGrid_Scroll_Width + ")")
  EndIf
  If IsGadget(*mg\RowScroll)
    ResizeGadget(*mg\RowScroll, X + W, Y, #MyGrid_Scroll_Width, H)
    ; debugMsg(sProcName, "(RowScroll) ResizeGadget(" + *mg\RowScroll + ", " + Str(X + W) + ", " + Y + ", " + #MyGrid_Scroll_Width + ", " + H + ")")
  EndIf    
  
  ResizeGadget(Gdt, X, Y , W, H)
  ; debugMsg(sProcName, "(Gdt) ResizeGadget(" + Gdt + ", " + X + ", " + Y + ", " + W + ", " + H + ")")
...
Procedure.i MyGrid_SetText(Gdt.i, Row.i, Col.i, Txt.s)
  ...
  ; If Col = 2 : debugMsg0(sProcName, "Row=" + Row + ", Col=" + Col + ", Txt=" + #DQUOTE$ + Txt + #DQUOTE$) : EndIf
...
Procedure.i MyGrid_AssignStyle(Gdt, GRow, GCol, Style)
  ...
  ; debugMsg(sProcName, "GRow=" + GRow + ", GCol=" + GCol + ", Style=" + Style)
...
;             Case #PB_Shortcut_PageDown  : MyGrid_MoveDown( Gdt, #MyGrid_Scroll_PageSize, #MyGrid_Move_Block)
            Case #PB_Shortcut_PageUp : MyGrid_MoveUp(Gdt, *mg\RowScrollPageSize, #MyGrid_Move_Block)
            Case #PB_Shortcut_PageDown  : MyGrid_MoveDown( Gdt, *mg\RowScrollPageSize, #MyGrid_Move_Block)
          EndSelect
        EndIf
...
;             Case #PB_Shortcut_PageDown  : MyGrid_MoveDown( Gdt, #MyGrid_Scroll_PageSize, #MyGrid_Move_Focus)
            Case #PB_Shortcut_PageUp : MyGrid_MoveUp(  Gdt, *mg\RowScrollPageSize, #MyGrid_Move_Focus)
            Case #PB_Shortcut_PageDown  : MyGrid_MoveDown( Gdt, *mg\RowScrollPageSize, #MyGrid_Move_Focus)
            ...
            Case #PB_Shortcut_Delete, #PB_Shortcut_Back
              ...
              If *mg\LstStyle()\Editable
                ...
                ; Debug "(c) calling _MyGrid_DrawCurrentCell(*mg)"
                _MyGrid_DrawCurrentCell(*mg)
              EndIf
              
            Case #PB_Shortcut_Return
              ; text input takes place in current cell regardless of mouse position
              ; Debug "#PB_Shortcut_Return: calling MyGrid_ShowCell(gdt, " + *mg\Row + ", " + *mg\Col + ")"
    ...
    Case #PB_EventType_Focus
      ...
      If _MyGrid_OverCellArea(*mg, x, y)
        ...
        row = *mg\Row
        col = *mg\Col
        ; Debug sProcName + " [#PB_EventType_Focus] row=" + row + ", *mg\Row=" + *mg\Row + ", *mg\Col=" + *mg\Col
        ; debugMsg(sProcName, "#PB_EventType_Focus: calling MyGrid_FocusCell(gdt, " + row + ", " + col + ")")
        ; Debug "(>>> eventtype_focus calling MyGrid_FocusCell"
    ...
    Case #PB_EventType_Input ;, #PB_EventType_LeftDoubleClick
      ...
      ; Debug "#PB_EventType_Input: calling MyGrid_ShowCell(gdt, " + *mg\Row + ", " + *mg\Col + ")"
      If MyGrid_ShowCell(gdt, *mg\Row, *mg\Col)
    ...
    Case #PB_EventType_LeftButtonUp
      ...
      Select mv
        Case #MyGrid_MouseMove_Nothing
          ...
          ; debugMsg(sProcName, "a simple click in a cell")
          row = _MyGrid_Row_Of_Y(*mg, y)
          col = _MyGrid_Col_Of_X(*mg, x)
          ; debugMsg(sProcName, "row=" + row + ", col=" + col + ", *mg\Row=" + *mg\Row + ", *mg\Col=" + *mg\Col)
    ...
    Case #PB_EventType_LeftButtonDown
      ...
      *mg\DownAreaRow = ar : *mg\DownAreaCol = ac
      If ar > 0 And ac > 0
        ...
        SelectElement(*mg\LstAreaRow() , ar)
        SelectElement(*mg\LstAreaCol() , ac)
        ; Debug "(>>> leftbuttondown calling MyGrid_FocusCell"
        MyGrid_FocusCell(gdt, *mg\LstAreaRow()\Row, *mg\LstAreaCol()\Col)
      EndIf
      
    Case #PB_EventType_RightButtonDown
      ; debugMsg(sProcName, "#PB_EventType_RightButtonDown")
...
; --------------------------- New Grid 
Procedure.i MyGrid_New(WinNbr, Gadget, X, Y, W, H, Rows = 500, Cols = 100, DoNotDraw = #False, NoColScrollBar = #False, NoRowScrollBar = #False)
  ; PROCNAMEC()
  ...
  *mg._MyGrid_Type = AllocateMemory(SizeOf(_MyGrid_Type))
  ...
  If NoRowScrollBar = #False
    W = W - #MyGrid_Scroll_Width
  EndIf
  If NoColScrollBar = #False
  ...
  *mg\Gadget = Gadget
  ...
  ; debugMsg(sProcName, "CanvasGadget=" + *mg\Gadget)
  ...
  Else
    ...
    *mg\ColScrollPageSize = #MyGrid_Scroll_PageSize
    BindGadgetEvent(*mg\ColScroll, @_MyGrid_SynchronizeGridCols(), #PB_All)
    ; debugMsg(sProcName, "ColScroll Gadget=" + *mg\ColScroll)
  EndIf
  ...
  Else
    ...
    *mg\RowScrollPageSize = #MyGrid_Scroll_PageSize
    BindGadgetEvent(*mg\RowScroll, @_MyGrid_SynchronizeGridRows(), #PB_All)
    ; debugMsg(sProcName, "RowScroll Gadget=" + *mg\RowScroll)
  EndIf
    ...
    Repeat
      ...
      Select Evnt
        ...
        Case #PB_Event_SizeWindow
          ; Debug "#PB_Event_SizeWindow calling MyGrid_Resize()"
